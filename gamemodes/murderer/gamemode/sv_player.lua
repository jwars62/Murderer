...
function GM:SaveArmMu()
	...
	if !text then
		local txt = ""
		file.Write("murder/ArmMu.txt", txt)
	end
end

function GM:LoadArmMu()
	local text = file.ReadDataAndContent("murder/ArmMu.txt")
	if text != "" then
		RandomMuWeapon = {}
		local i = 1
		for arme in text:gmatch("[^\r\n]+") do
			table.insert(RandomMuWeapon, arme)
		end
	end
end
...
function GM:PlayerLoadout(ply)
	...
	if ply:GetMurderer() then
		ply:Give( table.Random(RandomMuWeapon) )
...
function GM:ScalePlayerDamage( ply, hitgroup, dmginfo )
	-- Don't scale it dependings on hitgroup
end
	...
	if !ply:GetMurderer() then
		...
		if IsValid(attacker) && attacker:IsPlayer() then
			...
			elseif attacker != ply then
				...
				erm = attacker:GetActiveWeapon():GetClass()
				if erm != "weapon_mu_magnum" then
					attacker:StripWeapon(erm)
				end
				if ply:HasWeapon("weapon_e_poorcross") then
					ply:Give("sent_friendgnome")
				end
				attacker:SetTKer(true)
			
			else
				et = table.Random(rein)
				if et == "sup" then 
					et = table.Random(reins)
				end
				attacker:Give(et)
			end
		else
