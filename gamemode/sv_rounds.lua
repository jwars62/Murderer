...
local defaultMapList = file.Find("maps/*.bsp", "GAME")
...
local pref = {"md_","mu_","gm_","ttt_","ph_","nz_",}
...
function GM:LoadMapList() 
	...
	if jason then
		...
		for k, map in pairs(defaultMapList) do
			for i, p in pairs(pref) do
					if string.find(map, "^" .. p) then
						table.insert(tbl, map)
					end
			end
		end
